...
//jQuery(window).load(function() {
		...
		*/
		...
		jQuery.ajax({
				url : '/inventarios/establecer_modulo',
		        data:{
		        	hash_url:hash_url
		        },
		        type : 'POST',
...
////////////////////////////////////
    ...
    jQuery("#producto_totales, #composicion_totales, #ancho_totales, #color_totales, #proveedor_totales").on('change', function(e) {
		...
		if  ( (hash_url=="/inventarios/consulta_totales") )   {  
	...
	function cargarDependencia_totales(campo,val_prod,val_comp, val_ancho, val_color,val_proveedor,dependencia,nombre) {
		...
		var url = '/inventarios/cargar_dependencia_totales';	
		jQuery.ajax({
		        url : '/inventarios/cargar_dependencia_totales',
		        data:{
		        	campo:campo,
	...
	jQuery('#tabla_consulta_totales').dataTable( {
	  ...
	  "pagingType": "full_numbers",
		...
		"serverSide": true,
		"ajax": {
	            	"url" : "/inventarios/procesando_consulta_totales",
	         		"type": "POST",
				...
				success: function(data){
					if(data != true){
						...
						jQuery('html,body').animate({
							...
							spinner.stop();
							jQuery('#foo').css('display','none');
							window.location.href = '/inventarios/'+$catalogo;	
					}
				} 
			});
			return false;
...
/////////////////////////buscar proveedores consulta
	...
	var consulta_proveedor_consulta = new Bloodhound({
	  ...
	  remote: {
	        url: '/inventarios/catalogos/buscador?key=%QUERY',
	        replace: function () {
...
buscar_producto_cons.initialize();
	...
	function cargarDependencia_producto(valor) {
		...
		var url = '/inventarios/cargar_dependencia_producto';
		var dependencia = 'color';	

		jQuery.ajax({
		        url : '/inventarios/cargar_dependencia_producto',
		        data:{
		        	valor:valor,
		        },
...
///////////////////////////////
	...
	jQuery('#tabla_consulta_producto').dataTable( {
	  ...
	  "pagingType": "full_numbers",
		...
		"serverSide": true,
		"ajax": {
	            	"url" : "/inventarios/procesando_consulta_producto",
	         		"type": "POST",
	...
	jQuery('#tabla_consulta_proveedor').dataTable( {
	  ...
	  "pagingType": "full_numbers",
		...
		"serverSide": true,
		"ajax": {
	            	"url" : "/inventarios/procesando_consulta_proveedor",
	         		"type": "POST",
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    jQuery("#producto_catalogo, #color_catalogo, #composicion_catalogo, #calidad_catalogo").on('change', function(e) {
		...
		if  ( (hash_url=="/inventarios/productos") )   {  
	...
	function cargarDependencia_catalogo(campo,val_prod,val_color,val_comp,val_calida,dependencia,nombre) {
		...
		var url = '/inventarios/cargar_dependencia_catalogo';	

		jQuery.ajax({
		        url : '/inventarios/cargar_dependencia_catalogo',
		        data:{
		        	campo:campo,
...
//jQuery('input[type="checkbox"][class="check_activo"]').click(function(e) {
	jQuery('body').on('click','input.check_activo[type="checkbox"]', function (e) {   		
		...
		jQuery.ajax({
				        url : '/inventarios/marcando_activo',
				        data : { 
	...
	jQuery('#tabla_cat_productos').dataTable( {
	  ...
	  "pagingType": "full_numbers",
		...
		"serverSide": true,
		"ajax": {
	            	"url" : "/inventarios/procesando_cat_producto",
	         		"type": "POST",
 					"data": function ( d ) {
         		 	   if (comienzo) {
         		 	   	 d.start=0;	 //comienza en cero siempre q cambia de botones
		...
		"columnDefs": [
...

		            {
		               ...
		               if (row[6]==0) { 	
							texto='<td>';
								texto+='<a href="/inventarios/editar_producto/'+jQuery.base64.encode(row[5])+'" type="button"'; 
		            ...
		            
		            {
		                "render": function ( data, type, row ) {
		                	...
		                	if (row[6]==0) {
	   							texto='	<td>';								
									texto+=' <a href="/inventarios/eliminar_producto/'+(row[5])+'/'+jQuery.base64.encode(row[0])+ '"'; 
...

		            {
		                "render": function ( data, type, row ) {
						texto='	<td>';
							texto+=' <a href="/inventarios/cambiar_producto/'+jQuery.base64.encode(row[5])+'" type="button" ';
							texto+='	class="btn btn-warning btn-sm btn-block" >';
							texto+='	<span class="glyphicon glyphicon-edit"></span>';
							texto+='</a>';
						texto+='</td>';	 
...

		            {
		                "render": function ( data, type, row ) {
						texto='	<td>';
							texto+=' <a href="/inventarios/detalle_producto/'+jQuery.base64.encode(row[5])+'" type="button" ';
							texto+='	class="btn btn-warning btn-sm btn-block" >';
...
/////////////////////////////////////new////////////////////////////
    ...
    jQuery("#id_almacen").on('change', function(e) {
		var hash_url = window.location.pathname;
		if  ( (hash_url=="/inventarios/salidas") )   {  
				comienzo=true; //para indicar que start comience en 0;
...
jQuery('body').on('click','#conf_entrada', function (e) {
		...
		jQuery.ajax({
		        url : '/inventarios/validar_proceso',
		        data : { 
		        		  dato: "valor",
						...
						jQuery('#messages').css('display','none');
								...
								jQuery.ajax({
									        ...
									        success : function(dato) {	
												...
												$catalogo = e.target.name;
												window.location.href = '/inventarios/procesar_entrar/'+jQuery.base64.encode(data.num_mov)+'/'+jQuery.base64.encode(jQuery("#id_factura").val())+'/'+jQuery.base64.encode(jQuery("#id_estatus").val());
...
jQuery('body').on('click','#conf_devolucion', function (e) {
		...
		jQuery.ajax({
		        url : '/inventarios/validar_conf_devolucion',
		        data : { 
		        	dato: "valor"
		        },
		        type : 'POST',
						...
						jQuery('#messages').css('display','none');
								...
								jQuery.ajax({
									        ...
									        success : function(data) {	
												...
												$catalogo = e.target.name;
												window.location.href = '/inventarios/procesar_devoluciones/'+jQuery.base64.encode("-1");
  ...
  jQuery("#id_lote").on('change', function(e) {
 		 ...
 		 var val_calida = jQuery('#calidad').val();  		  
    		...
    		if  ((val_calida != "0") && (val_calida != "") && (val_calida != null)) 
    		{
				...
				var url = '/inventarios/id_proveedor';	
				jQuery.ajax({  //para tomar la referencia del producto
				        url : '/inventarios/refe_producto',
					    data:{
...
jQuery('#tabla_devolucion').dataTable( {
		...
		"serverSide": true,
		"ajax": {
	            	"url" : "/inventarios/procesando_servidor_devolucion",
	         		"type": "POST",
	         		 "data": function ( d ) {
		...
		"columnDefs": [
				...
				{ 
		            ...
		            {
		                "render": function ( data, type, row ) {
    					 texto='<td><a href="/inventarios/quitar_devolucion/'+jQuery.base64.encode(row[0])+'/'+jQuery.base64.encode(row[1])+ '"';  //+jQuery.base64.encode(row[6])+'" '; 
...
jQuery('#tabla_reporte').dataTable( {
	...
	"serverSide": true,
	"ajax": {
            	"url" : "/inventarios/procesando_reporte",
         		"type": "POST",
         		 "data": function ( d ) {
						if (comienzo) {
							 d.start=0;	 //comienza en cero siempre q cambia de botones
 ...
 "rowCallback": function( row, data ) {
    ...
    jQuery('#tabla_reporte tbody').on('click', 'td.details-control', function () {
        ...
        if ( row.child.isShown() ) { //si la fila esta "abierta" entonces "cerrarla"
            ...
            tr.removeClass('shown');
                 ...
                 $.ajax({
                        url: "/inventarios/procesando_detalle_reporte",
                        type: 'POST',
                        dataType: "json",
                        data: {
                        	   identificador : d[0],
  ...
  jQuery("#producto_rep, #color_rep, #composicion_rep, #calidad_rep").on('change', function(e) {
        ...
        if  ( (hash_url=="/inventarios/") || (hash_url=="/inventarios/reportes") )   {  
                comienzo=true; //para indicar que start comience en 0;
                var oTable =jQuery('#tabla_reporte').dataTable();
                oTable._fnAjaxUpdate();
        }   
  ...
  function cargarDependencia_reportes(campo,val_prod,val_color, val_comp, val_calidad,dependencia,nombre) {
        jQuery.ajax({
                url : '/inventarios/cargar_dependencia_reporte',
                data:{
                    campo:campo,
...
//////////////////Comienzo de tratamiento de dependencia///////////////////////////
    ...
    jQuery("#producto, #color, #composicion, #calidad").on('change', function(e) {
		...
		var campo = jQuery(this).attr("name");   
         ...
         var nombre = jQuery(this).attr("nombre");           //color composicion
        //alert(valor);
    	if ((dependencia !="") && (hash_url!="/inventarios/costo_inventario")) {	    
	        //limpiar la dependencia
	        jQuery("#"+dependencia).html(''); 
	        //cargar la dependencia
		...
		if  ( (hash_url=="/inventarios/costo_rollo") )   {  
		...
		if  ( (hash_url=="/inventarios/reportes") )   {  
		...
		if  ( (hash_url=="/inventarios/devolucion") )   {  //actualizar la regilla de abajo
		...
		if ((campo == 'calidad') && ( (hash_url=="/inventarios/entradas") || (hash_url=="/inventarios/editar_inventario") || (hash_url=="/inventarios/devolucion") ) ) { //si calidad cambio de valor
    		if  ((val_calida != "0") && (val_calida != "") && (val_calida != null)) 
    		{
				...
				jQuery.ajax({  //para tomar la referencia del producto
						...
						if  ( (hash_url!="/inventarios/editar_inventario") && (hash_url!="/inventarios/devolucion") )   {  	
	...
	function cargarDependencia(campo,val_prod,val_color,val_comp,val_calida,dependencia,nombre) {
		...
		var url = '/inventarios/cargar_dependencia';	

		jQuery.ajax({
		        url : '/inventarios/cargar_dependencia',
		        data:{
		        	campo:campo,
...
jQuery('#exportar_reporte').click(function (e) {
	...
	jQuery.ajax({
		        url : '/inventarios/exportar_reporte',
		        data : { 
					extra_search 	: jQuery("#botones").val(), 
					id_estatus 	 	: jQuery("#id_estatuss").val(), 
					//id_almacen 	 	: jQuery("#id_almacen_reporte").val(),
...
jQuery('body').on('click','.apartar', function (e) {
	...
	jQuery.ajax({
		        url : '/inventarios/marcando_apartado',
		        data : { 
		        	identificador: identificador,
		...
		jQuery('#myModalInicio').on('shown.bs.modal', function() {
				 ...
				 jQuery('body').on('change','#id_color_grupo', function (e) {
				           ...
				           var imagen = jQuery("#id_color_grupo option:selected").attr('imagen');
				            ...
				            var url= '/inventarios/uploads/productos/thumbnail/300X300/'+imagen.substr(0, (imagen.lastIndexOf(".")))+'_thumb'+imagen.substr(imagen.lastIndexOf(".")); 
				...
				jQuery('#tabla_producto_color').dataTable( {
					...
					"serverSide": true,
					"ajax": {
				            	"url" : "/inventarios/procesando_producto_color",
				         		"type": "POST",
								"data": function ( d ) {
...
jQuery('#tabla_producto_color2').dataTable( {
					...
					"serverSide": true,
					"ajax": {
				            	"url" : "/inventarios/procesando_producto_color2",
				         		"type": "POST",
								"data": function ( d ) {
								       d.grupo = jQuery("#grupo_oculto").val();  
								       d.id_color = jQuery("#id_color_grupo").val();  
...
jQuery('#tabla_inicio').dataTable( {
	...
	"serverSide": true,
	"ajax": {
            	"url" : "/inventarios/procesando_inicio",
         		"type": "POST",
				"data": function ( d ) {
			    ...
			    { //+'"
	                "render": function ( data, type, row ) {
	                	if (data) {
                                        ...
                                        url='/inventarios/uploads/productos/thumbnail/300X300/'+data[0].substr(0, (data[0].lastIndexOf(".")))+'_thumb'+data[0].substr(data[0].lastIndexOf(".")); 
                                        ...
                                        prod+= '<a href="/inventarios/detalles_grupo/'+jQuery.base64.encode(data[2])+'/'+jQuery.base64.encode(jQuery('#id_almacen_inicio').val())+'" class="thumbnail col-md-12 col-lg-12 col-xs-12" data-toggle="modal" data-target="#myModalInicio">';
...
jQuery('body').on('click','.prorrogar_venta', function (e) {
	...
	jQuery.ajax({
		        url : '/inventarios/marcando_prorroga_venta',
		        data : { 
...
jQuery('body').on('click','#conf_apartado', function (e) {
	...
	jQuery.ajax({
		        url : '/inventarios/apartado_definitivo',
		        data : { 
...
jQuery('#id_estatus, #id_color').change(function(e) {
	var hash_url = window.location.pathname;
	if  ( (hash_url!="/inventarios/entradas") )  {  //sino es entrada
		var oTable =jQuery('#tabla_inicio').dataTable();
		oTable._fnAjaxUpdate();
	}	
});
...
*/
	...
	"serverSide": true,
	"ajax": {
            	"url" : "/inventarios/procesando_apartado_pendiente",
         		"type": "POST",
				"data": function ( d ) {
   ...
   "columnDefs": [
    			...
    			{ 
	                "render": function ( data, type, row ) {
    					 texto='<td><a href="/inventarios/apartado_detalle/'+jQuery.base64.encode(row[6])+'/'+jQuery.base64.encode(row[7])+'/'+jQuery.base64.encode(jQuery('#id_almacen_pedido option:selected').val())+'/'+jQuery.base64.encode(row[11])+'/'+jQuery.base64.encode(row[15])+'/'+jQuery.base64.encode(row[16])+'" '; 
    			...
    			{ 
	                "render": function ( data, type, row ) {
	                	if (row[14]!=3) {
							texto='<td><a href="/inventarios/eliminar_apartado_detalle/'+jQuery.base64.encode(row[6])+'/'+jQuery.base64.encode(row[7])+'/'+jQuery.base64.encode(jQuery('#id_almacen_pedido option:selected').val())+'/'+jQuery.base64.encode(row[11])+'/'+jQuery.base64.encode(row[15])+'/'+jQuery.base64.encode(row[16])+'" '; 
								texto+='class="btn btn-danger  btn-block" data-toggle="modal" data-target="#modalMessage">';
								texto+='<span class="glyphicon glyphicon-remove"></span>';
							texto+='</a></td>';
						} else {
...
jQuery('#tabla_detalle').dataTable( {
	...
	"serverSide": true,
	"ajax": {
            	"url" : "/inventarios/procesando_detalle",
         		"type": "POST",
         		 "data": function ( d ) {
	...
	"infoCallback": function( settings, start, end, max, total, pre ) {
	    ...
	    if (settings.json.datos) {
		    ...
		    */
			...
			window.location.href = '/inventarios/pedidos';	
		}
	    return pre
	},    
...
jQuery('body').on('click','#incluir_salida', function (e) {
	...
	jQuery.ajax({
		        url : '/inventarios/incluir_apartado',
		        data : { 
...
jQuery('body').on('click','#excluir_salida', function (e) {
	...
	jQuery.ajax({
		        url : '/inventarios/excluir_apartado',
		        data : { 
		        	id_usuario: id_usuario,
		        	id_cliente: id_cliente,
		        	id_almacen:jQuery('#id_almacen_pedido').val(),
...
jQuery('body').on('click','#proc_salida_apartado', function (e) {
	...
	var spinner = new Spinner(opts).spin(target);
	 ...
	 var url = '/inventarios/confirmar_proc_apartado_sino';
							...
							//alert('ok');
								...
								jQuery.ajax({
									        ...
									        success : function(dato) {	
												...
												var url = "/inventarios/proc_apartado_pedido_definitivo/"+jQuery.base64.encode(num_mov)+'/'+jQuery.base64.encode(data.id_cargador)+'/'+jQuery.base64.encode(id_tipo_pedido)+'/'+jQuery.base64.encode(id_tipo_factura)+'/'+jQuery.base64.encode(id_almacen);
...
jQuery('body').on('click','#proc_salida_pedido', function (e) {
	...
	var spinner = new Spinner(opts).spin(target);
	 ...
	 var url = '/inventarios/confirmar_proc_pedido_sino';
							...
							//alert('ok');
								...
								jQuery.ajax({
									        ...
									        success : function(dato) {	
												...
												valor= jQuery.base64.encode(data.valor);
												var url = "/inventarios/proc_salida_pedido_definitivo/"+jQuery.base64.encode(num_mov)+'/'+jQuery.base64.encode(data.id_cargador)+'/'+jQuery.base64.encode(id_tipo_pedido)+'/'+jQuery.base64.encode(id_tipo_factura)+'/'+jQuery.base64.encode(id_almacen);
												jQuery('#modalMessage').modal({
													  show:'true',
													remote:url,
												}); 									        	
...
jQuery('body').on('click','#proc_salida', function (e) {
	...
	id_tipo_factura = (id_tipo_pedido==2) ? 0:jQuery("#id_tipo_factura_salida").val();
	 ...
	 var url = '/inventarios/confirmar_salida_sino';
							...
							jQuery('#foo').css('display','none');
								...
								jQuery.ajax({
									        ...
									        success : function(dato) {	
												...
												var url = "/inventarios/pro_salida/"+valor+'/'+data.id_cliente+'/'+jQuery.base64.encode(id_almacen)+'/'+jQuery.base64.encode(id_tipo_pedido)+'/'+jQuery.base64.encode(id_tipo_factura);
...
jQuery('table').on('click','.agregar', function (e) {
	...
	jQuery.ajax({
		        url : '/inventarios/agregar_prod_salida',
		        data : { 
		        	identificador: identificador,
		        	id_cliente:proveedor,
...
jQuery('#tabla_entrada').dataTable( {
	...
	"serverSide": true,
	"ajax": {
            	"url" : "/inventarios/procesando_servidor",
         		"type": "POST",
         		 "data": function ( d ) {
...
jQuery('table').on('click','.quitar', function (e) {
	...
	jQuery.ajax({
		        url : '/inventarios/quitar_prod_salida', //
		        data : { 
		        	identificador: identificador,
		        	id_tipo_pedido: id_tipo_pedido,
		        	id_tipo_factura: id_tipo_factura,
...
jQuery('#pedido_completo_detalle').dataTable( {
	...
	"serverSide": true,
	"ajax": {
            	"url" : "/inventarios/procesando_completo_detalle",
         		"type": "POST",
         		 "data": function ( d ) {
...
jQuery('#tabla_pedido_completado').dataTable( {
	...
	"serverSide": true,
	"ajax": {
            	"url" : "/inventarios/procesando_pedido_completo",
         		"type": "POST",
    			"data": function ( d ) {
						    d.id_almacen = jQuery('#id_almacen_pedido').val();	
    			 }          		
   ...
   "columnDefs": [
    			...
    			{ 
	                "render": function ( data, type, row ) {
    					 texto='<td><a href="/inventarios/pedido_completado_detalle/'+jQuery.base64.encode(row[5])+'/'+jQuery.base64.encode(row[6])+'/'+jQuery.base64.encode(jQuery('#id_almacen_pedido option:selected').val())+'/'+jQuery.base64.encode(row[8])+'/'+jQuery.base64.encode(row[11])+'/'+jQuery.base64.encode(row[12])+'" '; 
						 	texto+=' class="btn btn-success btn-block">';
						 	texto+=' Detalles';
						 texto+='</a></td>';
...
jQuery('body').on('click','#incluir_pedido', function (e) {
     ...
     num_mov = jQuery("#num_mov").val();  //3; //
 	 jQuery.ajax({
		        url : '/inventarios/incluir_pedido',
		        data : { 
		        	num_mov: num_mov,
		        id_almacen:jQuery('#id_almacen_pedido').val(),	
		        id_tipo_pedido:jQuery('#id_tipo_pedido').val(),
...
num_mov = jQuery("#num_mov").val();  //3; //
	...
	jQuery.ajax({
		        url : '/inventarios/excluir_pedido',
		        data : { 
		        	num_mov: num_mov,
		        	id_almacen:jQuery('#id_almacen_pedido').val(),
...
jQuery('#pedido_detalle').dataTable( {
	...
	"serverSide": true,
	"ajax": {
            	"url" : "/inventarios/procesando_pedido_detalle",
         		"type": "POST",
         		 "data": function ( d ) {
...
jQuery('body').on('click','.prorrogar_tienda', function (e) {
	...
	jQuery.ajax({
		        url : '/inventarios/marcando_prorroga_tienda',
		        data : { 
		        	id_cliente_apartado: id_cliente_apartado,
		        	id_almacen:jQuery('#id_almacen_pedido').val(),
		        },
...
jQuery('#tabla_pedido').dataTable( {
	...
	"serverSide": true,
	"ajax": {
            	"url" : "/inventarios/procesando_pedido_pendiente",
         		"type": "POST",
				"data": function ( d ) {
						    d.id_almacen = jQuery('#id_almacen_pedido').val();	
    			 }          		
   ...
   "columnDefs": [
    			...
    			{ 
	                "render": function ( data, type, row ) {
    					 texto='<td><a href="/inventarios/pedido_detalle/'+jQuery.base64.encode(row[6])+'/'+jQuery.base64.encode(jQuery('#id_almacen_pedido option:selected').val())+'/'+jQuery.base64.encode(row[12])+'/'+jQuery.base64.encode(row[13])+'" '; 
						 	texto+=' class="btn btn-success btn-block">';
						 	texto+=' Detalles';
						 texto+='</a></td>';
    			...
    			{ 
	                "render": function ( data, type, row ) {
	                	if ( (row[11]!=6) ) { //|| ( (row[11]==6) && (jQuery("#mi_perfil").val() !='4')  )  
							texto='<td><a href="/inventarios/eliminar_pedido_detalle/'+jQuery.base64.encode(row[6])+'/'+jQuery.base64.encode(jQuery('#id_almacen_pedido option:selected').val())+'/'+jQuery.base64.encode(row[12])+'/'+jQuery.base64.encode(row[13])+'" '; 
								texto+='class="btn btn-danger  btn-block" data-toggle="modal" data-target="#modalMessage">';
								texto+='<span class="glyphicon glyphicon-remove"></span>';
							texto+='</a></td>';
						} else {
...
jQuery('body').on('click','#conf_pedido', function (e) {
	...
	jQuery.ajax({
		        url : '/inventarios/pedido_definitivo',
		        data : { 
		        	num_mov: num_mov,
		        	id_tipo_pedido:id_tipo_pedido,
		        	id_tipo_factura:id_tipo_factura,
    ...
    jQuery("#producto_pedido, #composicion_pedido, #ancho_pedido, #color_pedido, #proveedor_pedido").on('change', function(e) {
		...
		if  ( (hash_url=="/inventarios/generar_pedidos") )   {  
	...
	function cargarDependencia_pedido(campo,val_prod,val_comp, val_ancho, val_color,val_proveedor,dependencia,nombre) {
		...
		var url = '/inventarios/cargar_dependencia_pedido';	
		jQuery.ajax({
		        url : '/inventarios/cargar_dependencia_pedido',
		        data:{
		        	campo:campo,
...
jQuery('#pedido_entrada').dataTable( {
	...
	"serverSide": true,
	"ajax": {
            	"url" : "/inventarios/procesando_pedido_entrada",
         		"type": "POST",
         		 "data": function ( d ) {
         		 	   if (comienzo) {
         		 	   	 //d.start=0;	 //comienza en cero siempre q cambia de botones
	...
	"columnDefs": [
				...
				{ 
	                "render": function ( data, type, row ) {
                            ...
                            prod= '<a href="/inventarios/detalles_imagen/'+jQuery.base64.encode(row[14])+'/'+jQuery.base64.encode(row[1])+'" data-toggle="modal" data-target="#myModaldashboard">';
                               prod+= row[2];
                            prod+='</a>';
...
jQuery('#pedido_salida').dataTable( {
	...
	"serverSide": true,
	"ajax": {
            	"url" : "/inventarios/procesando_pedido_salida",
         		"type": "POST",
     }, 
...
"infoCallback": function( settings, start, end, max, total, pre ) {
	...
	"columnDefs": [
				...
				{ 
	                "render": function ( data, type, row ) {
                            prod= '<a href="/inventarios/detalles_imagen/'+jQuery.base64.encode(row[13])+'/'+jQuery.base64.encode(row[1])+'" data-toggle="modal" data-target="#myModaldashboard">';
                               prod+= row[2];
                            prod+='</a>';
...
jQuery('table').on('click','.agregar_pedido', function (e) {
	...
	jQuery.ajax({
		        url : '/inventarios/agregar_prod_pedido',
		        data : { 
		        	identificador: identificador,
		        	movimiento: movimiento,
		        	id_tipo_pedido: id_tipo_pedido,
...
jQuery('table').on('click','.quitar_pedido', function (e) {
	...
	identificador = (jQuery(this).attr('identificador'));
	jQuery.ajax({
		        url : '/inventarios/quitar_prod_pedido', //
		        data : { 
		        	identificador: identificador
		        },
		        type : 'POST',
...
jQuery('#tabla_productos').dataTable( {
	...
	"serverSide": true,
	"ajax": {
            	"url" : "/inventarios/procesando_productos_temporales",
         		"type": "POST"
     },   
	"infoCallback": function( settings, start, end, max, total, pre ) {
	    if (settings.json.totales) {
   ...
   "columnDefs": [
    			...
    			{ 
	                "render": function ( data, type, row ) {
						texto='<td>';
						    texto+='<a href="/inventarios/eliminar_prod_temporal/'+(row[0])+'/'+jQuery.base64.encode(row[1])+'" '; 
									texto+='class="btn btn-danger btn-sm btn-block" data-toggle="modal" data-target="#modalMessage"> ';
									texto+='<span class="glyphicon glyphicon-remove"></span> ';
							texto+='</a>';
						texto+='</td>';
...
jQuery('body').on('click','#impresion', function (e) {
	...
	codigo = jQuery("#editar_prod_inven").val(); 
		...
		var spinner = new Spinner(opts).spin(target);
		jQuery.ajax({
		        url : '/inventarios/validar_impresion',
		        data : { 
		        	codigo: codigo,
		        },
		        type : 'POST',
	...
	jQuery('#tabla_cambio').dataTable( {
	  ...
	  "pagingType": "full_numbers",
		...
		"serverSide": true,
		"ajax": {
	            	"url" : "/inventarios/procesando_servidor_cambio",
	         		"type": "POST",
	         		 "data": function ( d ) {
	     				   d.codigo = jQuery("#codigo_original").val(); 
	    			 }
...
*/
	...
	function cargarDependencia_estatus(val_prod,id_estatus) {
		...
		var url = '/inventarios/cargar_dependencia_estatus';	
		jQuery.ajax({
		        url : url,
		        data:{
		        	val_prod:val_prod,
			...
			success: function(data){
				if(data != true){
					...
					jQuery('html,body').animate({
						...
						spinner.stop();
						jQuery('#foo').css('display','none');
						window.location.href = '/inventarios/usuarios';						
				}
			} 
		});
		return false;
...
//gestion de usuarios (crear, editar y eliminar )
			...
			success: function(data){
				...
				if(data.estado.exito != true){
					...
					e.preventDefault();
					 ...
					 window.location.href = '/inventarios/'+data.dato.valor+'#mante';	
					 window.location.reload();
					 return false;
	...
	function cargardependientes(valor_padre,elem_hijo) {
		...
		var url = '/inventarios/unidades/cargar_elemhijo';	
		//alert(elem_hijo);
